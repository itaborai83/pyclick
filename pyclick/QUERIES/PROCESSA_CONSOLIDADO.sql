...
CREATE VIEW VW_DADOS_MEDICAO AS 
	...
	FROM	INCIDENTES AS A
			...
			--
	WHERE	A.ID_CHAMADO NOT LIKE 'T%'
	AND		(H.ID_CHAMADO IS NULL OR (H.ID_CHAMADO IS NOT NULL AND COALESCE(H.ESTORNO, 'N') <> 'S' ))
	ORDER	BY A.ID_CHAMADO;
...
CREATE VIEW VW_DADOS_MEDICAO_FALTANDO AS 
	...
	OR		(A.STATUS <> 'EM ABERTO' AND CATEGORIA IS NULL)
	OR		(A.STATUS <> 'EM ABERTO' AND CATEGORIA IS NOT NULL AND MESA IS NULL)
	OR		(A.STATUS <> 'EM ABERTO' AND CATEGORIA IS NOT NULL AND MESA IN (SELECT MESA FROM MESAS_N4_SAP) AND PESO IS NULL)
	OR		(A.STATUS <> 'EM ABERTO' AND CATEGORIA IS NOT NULL AND MESA IN (SELECT MESA FROM MESAS_N4_SAP) AND PESO IS NOT NULL AND PESO IS NULL)
	OR		(A.STATUS NOT IN ('EM ABERTO', 'CANCELADO') AND CATEGORIA IS NOT NULL AND MESA IN (SELECT MESA FROM MESAS_N4_SAP) AND PESO IS NOT NULL AND PESO IS NOT NULL AND DURACAO IS NULL))
	ORDER	BY A.ID_CHAMADO;
