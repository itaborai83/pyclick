...
CREATE VIEW VW_DURACOES AS 
	WITH ULTIMAS_MESAS AS (
		...
		FROM	(
					SELECT	ID_CHAMADO, MAX(ID_ACAO) AS MAX_ID_ACAO
		...
		--
		WHERE	B.MESA_ATUAL IN (SELECT MESA FROM MESAS_N4_SAP)
		--AND		B.MESA_ATUAL LIKE '%PRIOR%'
	),
	SOMAS AS (
		SELECT	A.ID_CHAMADO
		,		A.CHAMADO_PAI 
		...
		,		SUM(CASE	WHEN 	B.MESA = 'N4-SAP-SUSTENTACAO-PRIORIDADE' 
							THEN 	(CASE 	WHEN C.MESA_ATUAL = 'N4-SAP-SUSTENTACAO-PRIORIDADE' 
											THEN C.DURACAO_M 
											ELSE 0 
									END)
				...
				END) AS DURACAO
		,		SUM(CASE	WHEN B.MESA = 'N4-SAP-SUSTENTACAO-PRIORIDADE'
							THEN (CASE WHEN C.MESA_ATUAL = 'N4-SAP-SUSTENTACAO-PRIORIDADE' THEN 1 ELSE 0 END)
							WHEN (B.MESA IN (SELECT MESA FROM MESAS_N4_SAP)) THEN 1
							ELSE 0 
				END) AS QTD
		FROM	INCIDENTES AS A
				--
				INNER JOIN ULTIMAS_MESAS AS B
				ON 	A.ID_CHAMADO = B.ID_CHAMADO 
				--
				INNER JOIN INCIDENTE_ACOES AS C
				ON	A.ID_CHAMADO = C.ID_CHAMADO
				--
		WHERE	C.MESA_ATUAL 		IN (SELECT MESA FROM MESAS_N4_SAP)
		AND		C.PENDENCIA 		= 'N'
		AND		C.DURACAO_M 		> 0
	...
	SELECT	A.ID_CHAMADO
	,		COALESCE(B.DURACAO, A.DURACAO, 0) AS DURACAO
	FROM	
			DURACOES AS A
			--
			LEFT OUTER JOIN INCIDENTES_OVERRIDE AS B
...
CREATE VIEW VW_KPI_PRP_DETALHES AS
	...
	AND		STATUS IN ('ENCERRADO', 'RESOLVIDO', 'CANCELADO', 'EM ABERTO')
	AND		PESO = 35
...
CREATE VIEW VW_KPI_PRO_DETALHES AS
	...
	,		CASE WHEN STATUS = 'EM ABERTO' THEN 1.0 ELSE 0.0 END  AS ABERTO
	,		0.0 AS ENCAMINHADO
	,		0.0 AS PRIORIZADO
	FROM	DADOS_MEDICAO AS A
	WHERE	1=1
	AND		A.STATUS IN ('ENCERRADO', 'RESOLVIDO', 'CANCELADO', 'EM ABERTO')
...
DROP VIEW IF EXISTS VW_KPI_IDS_DETALHES;
CREATE VIEW VW_KPI_IDS_DETALHES AS
	SELECT	A.ID_CHAMADO
	,		A.STATUS
	,		A.CATEGORIA
	,		A.MESA
	...
	,		A.DURACAO
	,		A.OBS_OVERRIDE
	--,		CEIL((A.DURACAO / 60.0) / 180) DURACAO_IDS
	,		CAST ( ((A.DURACAO / 60.0) / 180) AS INT ) 
	+ 		( ((A.DURACAO / 60.0) / 180) 
	> 		CAST ( ((A.DURACAO / 60.0) / 180) AS INT )) AS DURACAO_IDS -- WTF!!!
	FROM	VW_DADOS_MEDICAO AS A
	WHERE	A.MESA IN (SELECT MESA FROM MESAS_N4_SAP)
	AND		A.STATUS = 'EM ABERTO'
	AND		A.DURACAO > PRAZO
	ORDER	BY ID_CHAMADO;

DROP VIEW IF EXISTS VW_KPI_IDS;
CREATE VIEW VW_KPI_IDS AS
	SELECT	COUNT(*) AS INCIDENTES
	,		SUM(CASE WHEN DURACAO_IDS = 1 THEN 1 ELSE 0 END) AS IDS_0H_180H
	,		SUM(CASE WHEN DURACAO_IDS = 2 THEN 1 ELSE 0 END) AS IDS_180H_360H
	,		SUM(CASE WHEN DURACAO_IDS = 3 THEN 1 ELSE 0 END) AS IDS_360H_540H
	,		SUM(CASE WHEN DURACAO_IDS = 4 THEN 1 ELSE 0 END) AS IDS_540H_720H
	,		SUM(CASE WHEN DURACAO_IDS = 5 THEN 1 ELSE 0 END) AS IDS_720H_900H
	,		SUM(CASE WHEN DURACAO_IDS > 5 THEN 1 ELSE 0 END) AS IDS_GT_900H
	,		100.0 * (SUM(DURACAO_IDS + 0.0) / COUNT(*))  AS IDS
	FROM	VW_KPI_IDS_DETALHES;
