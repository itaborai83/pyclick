...
INCS AS (
	...
	*/ 	-- USE SINGLE LINE COMMENT AT COLUMN 0 TO TEST THE SUBQUERY
	...
	,		SECTN_DEPT_ID
	,		BLDNG_ROOM_ID
	FROM 	VW_INCIDENT INCIDENT WITH(NOLOCK)
	WHERE	1=1
			-- ID DE SISTEMA
	...
	*/
	AND		( 	(1=1 AND INCIDENT.STATUS_ENUM = 1 AND INCIDENT.INC_RESOLVE_ACT IS NULL) 						OR -- ABERTOS
				(1=1 AND INCIDENT.STATUS_ENUM IN (2, 3) AND INCIDENT.INC_RESOLVE_ACT  > (SELECT END_DT FROM PARAMS)) ) -- ENCERRADOS & RESOLVIDOS 
	--
	UNION
	--
...
*/			
	AND		( 	(1=1 AND INCIDENT.STATUS_ENUM IN (2, 3) AND INCIDENT.INC_CLOSE_DATE  BETWEEN (SELECT START_DT FROM PARAMS) AND (SELECT END_DT FROM PARAMS) ) OR -- ENCERRADOS 
				(1=1 AND INCIDENT.STATUS_ENUM IN (2, 3) AND INCIDENT.INC_RESOLVE_ACT BETWEEN (SELECT START_DT FROM PARAMS) AND (SELECT END_DT FROM PARAMS) ) )  -- RESOLVIDOS
),
ACTION_TYPES AS (
	SELECT	ACT_TYPE_ID 
	FROM 	ACT_TYPE WITH (NOLOCK)
...
ACTIONS AS (
	...
	,		ACT_REG.REMARKS	
	FROM	INCS WITH (NOLOCK)
			--
			INNER HASH JOIN VW_ACT_REG AS ACT_REG WITH (NOLOCK)
			ON	INCS.INCIDENT_ID =  ACT_REG.INCIDENT_ID
			AND	(INCS.STATUS_ENUM <> 1 AND ACT_REG.DATE_ACTIONED <= (SELECT END_DT FROM PARAMS)
			OR	INCS.STATUS_ENUM = 1 AND ACT_REG.DATE_ACTIONED BETWEEN (SELECT START_DT FROM PARAMS) AND (SELECT END_DT FROM PARAMS))
			--
...
REPORT AS (
	...
	,		(SELECT DATE_LOGGED 	FROM INCS WITH (NOLOCK) WHERE INCIDENT_ID = INCIDENT.INCIDENT_ID) AS 'Data Abertura Chamado'
	,		(SELECT INC_RESOLVE_ACT FROM INCS WITH (NOLOCK) WHERE INCIDENT_ID = INCIDENT.INCIDENT_ID) AS 'Data Resolucao Chamado'
	...
	,		CASE	WHEN ACT_TYPE.ACT_TYPE_ID IN 
					( SELECT ACT_TYPE_ID FROM CHANGE_FIELDS_ACTIONS WITH (NOLOCK))
					THEN ACT_REG.REMARKS ELSE NULL
	...
	,		CASE	WHEN ACT_TYPE.ACT_TYPE_ID = 1 
					THEN (	SELECT 	SERV_DEPT.SERV_DEPT_N 
							FROM 	ASSYST_USR WITH (NOLOCK)
									--
									INNER JOIN SERV_DEPT WITH (NOLOCK)
	...
	,		CASE	WHEN ACT_TYPE.ACT_TYPE_SC = (SELECT ACT_TYPE_SC FROM ACT_TYPE WITH (NOLOCK) WHERE ACT_TYPE_SC = 'ASSIGN') 
					THEN  ACT_REG.TIME_TO_RESOLVE
					ELSE NULL
			END 												AS 'Prazo Prioridade ANO (m)'
	,		CASE	WHEN ACT_TYPE.act_type_sc = (SELECT ACT_TYPE_SC FROM ACT_TYPE WITH (NOLOCK) WHERE ACT_TYPE_SC = 'SUPP-ASSIGN') THEN  ACT_REG.time_to_resolve
					ELSE NULL
	...
	,		CASE	WHEN ACT_TYPE.ACT_TYPE_SC = (SELECT ACT_TYPE_SC FROM ACT_TYPE WITH (NOLOCK) WHERE ACT_TYPE_SC = 'ASSIGN') THEN  ACT_REG.ASSIGNMENT_TIME
					ELSE NULL
			END 												AS 'Tempo Util Atribuicao Mesa (m)'
			-- Falta Tempo Total de Atribuicao CA (m)
	,		CASE	WHEN ACT_TYPE.ACT_TYPE_SC = (SELECT ACT_TYPE_SC FROM ACT_TYPE WITH (NOLOCK) WHERE ACT_TYPE_SC = 'SUPP-ASSIGN') THEN  ACT_REG.ASSIGNMENT_TIME
					ELSE NULL
			END 												AS 'Tempo Util Atribuicao CA (m)'
	,		CASE	WHEN (	SELECT COUNT(LINK_INC_ID) FROM LINK_INC WITH (NOLOCK)
							WHERE LINK_INC.INCIDENT_ID = INCIDENT.INCIDENT_ID) > 0 
					THEN 'Sim' ELSE 'Nao'
			END 												AS 'Vinculo'
	,		CASE 	WHEN (
						SELECT 	COUNT(LINK_INC.INCIDENT_ID) 
						FROM 	LINK_INC WITH (NOLOCK)
								--
