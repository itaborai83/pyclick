...
SET		DATA_FIM_ACAO = (
			SELECT 	DATA_FIM_ACAO 
			FROM 	DATAS_FIM 
			WHERE 	ID_CHAMADO 	= A.ID_CHAMADO
			AND		ID_ACAO		= A.ID_ACAO
		);

-- FIX LAST ACTION FLAG
UPDATE 	REL_MEDICAO AS A
SET 	ULTIMA_ACAO = CASE WHEN DATA_FIM_ACAO IS NULL THEN 'y' ELSE 'n' END;
...
CREATE VIEW VW_HORAS_UTEIS_ACOES AS
	WITH ACOES AS (
		...
		,		LEAD(A.DATA_INICIO_ACAO) OVER (PARTITION BY A.ID_CHAMADO ORDER BY A.ID_ACAO) AS PROX_DATA_ACAO
		/*
		,		COALESCE(
