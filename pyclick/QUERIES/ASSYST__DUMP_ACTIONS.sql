...
,		C.SERV_DEPT_N
,		COALESCE(D.ASSYST_USR_ID, 0)				AS ASS_USR_ID
,		D.ASSYST_USR_SC
,		D.ASSYST_USR_N
...
FROM 	VW_ACT_REG 									AS A		
		--
		INNER JOIN ACT_TYPE  						AS B
		ON 	A.ACT_TYPE_ID 							= B.ACT_TYPE_ID
		--
		LEFT OUTER JOIN SERV_DEPT					AS C
		ON	A.SERV_DEPT_ID							= C.SERV_DEPT_ID
		AND	A.ACT_TYPE_ID 							= 1 
		--
		LEFT OUTER JOIN ASSYST_USR					AS D
		ON	A.ASS_USR_ID							= D.ASSYST_USR_ID
		AND	A.ACT_TYPE_ID 							= 1
		--
WHERE	1=1
